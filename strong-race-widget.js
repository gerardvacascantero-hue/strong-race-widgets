(function() {
  ...
  if (!document.getElementById('sr-fonts')) {
    ...
    style.id = 'sr-fonts';
    style.textContent = `
      @font-face {font-family:'Panton';src:url('https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/panton-bold.otf') format('opentype');font-weight:700;font-style:normal}
      @font-face {font-family:'Panton';src:url('https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/panton-regular.otf') format('opentype');font-weight:400;font-style:normal}
      :root {
        --sr-ff:'Panton',system-ui,sans-serif;
        --sr-ac:#8ECDFA;--sr-bg:#000;--sr-card:#050505;--sr-tab:#161616;
        --sr-text:#fff;--sr-muted:#9a9a9a;--sr-border:#1e1e1e;
      }
      .sr-widget *,.sr-widget *::before,.sr-widget *::after{box-sizing:border-box;margin:0;padding:0}
      .sr-widget{width:100%;background:var(--sr-bg);color:var(--sr-text);font-family:var(--sr-ff);font-weight:400}
      ...
      .p3-was{font-weight:400;font-size:15px;color:var(--sr-muted);text-decoration:line-through}
      ...
      @media(max-width:640px){
        .p2-grid,.p3-grid{grid-template-columns:1fr}
        .p4-body{flex-direction:column;height:auto}
        .p4-list{width:100%;max-width:100%;min-width:unset;height:260px;border-left:none;border-top:1px solid var(--sr-border)}
        .p4-map{height:320px;flex:none}
      }
    `;
  ...
  // ── PASO 2 logic ─────────────────────────────────────────
  const FECHAS='261106/261109', AE_URL='https://www.aireuropa.com/es/vuelos';
  const CITIES=[
  ...
  // ── PASO 3 logic ─────────────────────────────────────────
  const HOTELS=[
    {badge:'-15% Strong Race',name:'Meliá Palma Marina',zone:'Paseo Marítimo',stars:4,desc:'Tan cerca de la ciudad como tan cerca del mar.',now:'Desde 128€',img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/melia-palma-marina.jpg',url:'https://events.melia.com/es/events/palma-port/Strong-Race0'},
    {badge:'-20% Strong Race',name:'INNSiDE Palma Bosque',zone:'Palma',stars:4,desc:'Descubre la bonita y cosmopolita ciudad de Palma.',now:'Desde 116€',img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/innside-palma-bosque.jpg',url:'https://events.melia.com/es/events/palma-port/Strong-Race0'},
    {badge:'-10% Strong Race',name:'INNSiDE Palma Center',zone:'Centro',stars:4,desc:'Diseño urbano en el cielo de Palma.',now:'Desde 141€',img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/innside-palma-center.jpg',url:'https://events.melia.com/es/events/palma-port/Strong-Race0'},
  ];
  ...
  function loadLeaflet(cb){
    ...
    const s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.js';s.onload=cb;document.head.appendChild(s);
  }
  loadLeaflet(function(){
    const POIS=[
      {n:'Velódromo Illes Balears',d:'SEDE de la carrera',ll:[39.6095,2.6712],venue:true,img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/velodromo-illes-balears.jpg'},
      {n:'Catedral de Mallorca (La Seu)',d:'El icono de Palma',ll:[39.5663,2.6486],img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/catedral-de-mallorca-la-seu.jpg'},
      {n:'Passeig del Born',d:'Compras, terrazas y tapas',ll:[39.5710,2.6478],img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/passeig-del-born.jpg'},
      {n:'Castell de Bellver',d:'Vistas 360º sobre la bahía',ll:[39.5636,2.6199],img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/castell-de-bellver.jpg'},
      {n:'Port de Sóller',d:'Pueblo costero · tranvía histórico',ll:[39.7958,2.6936],img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/port-de-soller.jpg'},
      {n:'Valldemossa',d:'Pueblo de la Tramuntana',ll:[39.7099,2.6225],img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/valldemossa.jpg'},
      {n:'Platja de Palma',d:'Recuperación activa con vistas',ll:[39.5169,2.7370],img:'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@main/assets/img/platja-de-palma.jpg'},
    ];
    const mapEl=document.getElementById('sr-map');
    const map=L.map(mapEl,{scrollWheelZoom:false,attributionControl:false}).setView([39.62,2.66],11);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:19}).addTo(map);
    new ResizeObserver(()=>map.invalidateSize()).observe(mapEl);
    function pinIcon(a){const c=a?'#fff':'#8ECDFA';return L.divIcon({className:'',iconSize:[28,38],iconAnchor:[14,38],popupAnchor:[0,-42],html:`<div style="filter:drop-shadow(0 3px 8px rgba(0,0,0,.6))"><svg width="28" height="38" viewBox="0 0 28 38" xmlns="http://www.w3.org/2000/svg"><path d="M14 0C6.27 0 0 6.27 0 14c0 10.5 14 24 14 24s14-13.5 14-24C28 6.27 21.73 0 14 0z" fill="${c}"/><circle cx="14" cy="14" r="5" fill="rgba(0,0,0,.3)"/></svg></div>`});}
    function popHtml(p){return`<img class="pp-img" src="${p.img}" alt="${p.n}"><div class="pp-bd"><div class="pp-t">${p.n}</div><div class="pp-d">${p.d}</div><a class="pp-lk" href="https://www.google.com/maps/search/${encodeURIComponent(p.n+' Mallorca')}" target="_blank" rel="noopener">Cómo llegar</a></div>`;}
    const mks={},its={},listEl=document.getElementById('sr-poi');
    POIS.forEach((p,i)=>{
