(function() {
  ...
  const A = window.SR_ASSETS_BASE || 'https://cdn.jsdelivr.net/gh/gerardvacascantero-hue/strong-race-widgets@v1.0.0/assets/';
  ...
  // ── Find mount point ─────────────────────────────────────
  // Framer y otros hosts React insertan el nodo tras hidratar, así que lo esperamos.
  whenMounted('strong-race-widget', init);

  function whenMounted(id, cb) {
    const found = document.getElementById(id);
    if (found) return cb(found);
    if (document.readyState === 'loading')
      return document.addEventListener('DOMContentLoaded', () => whenMounted(id, cb), {once:true});
    const obs = new MutationObserver(() => {
      const el = document.getElementById(id);
      if (el) { obs.disconnect(); clearTimeout(timer); cb(el); }
    });
    obs.observe(document.documentElement, {childList:true, subtree:true});
    const timer = setTimeout(() => {
      obs.disconnect();
      console.error('[StrongRace] No se encontró #strong-race-widget tras 10 s');
    }, 10000);
  }
  ...
  const HOTELS=[
    ...
    {badge:'-20% Strong Race',name:'INNSiDE Palma Bosque',zone:'Palma',stars:4,desc:'Descubre la bonita y cosmopolita ciudad de Palma.',now:'Desde 116€',img:A+'hotels/innside-palma-bosque.jpg',url:'https://events.melia.com/es/events/palma-port/Strong-Race0'},
    {badge:'-10% Strong Race',name:'INNSiDE Palma Center',zone:'Centro',stars:4,desc:'Diseño urbano en el cielo de Palma.',now:'Desde 141€',img:A+'hotels/innside-palma-center.jpg',url:'https://events.melia.com/es/events/palma-port/Strong-Race0'},
  ];
